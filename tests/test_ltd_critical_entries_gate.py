...
def test_ltd_critical_entries_gate_passes_for_expected_inventory_and_env() -> None:
    markdown = """
| `1min.AI` | `Advanced Business Plan` | `12 licenses / 12 accounts` | `Owned` |  | `Tier 1` | Worker-owned credential pool, durable PropertyQuarry queue, principal-bound provider receipts, and private first-party asset materialization | PropertyQuarry generation remains completed after exactly one real `1min` image call. |
...
def test_ltd_critical_entries_gate_fails_closed_on_missing_inventory_or_env() -> None:
    ...
    assert "onemin_provider_call_discovery" in receipt["failures"]
    ...
    assert "onemin_local_credential_slot" in receipt["failures"]
