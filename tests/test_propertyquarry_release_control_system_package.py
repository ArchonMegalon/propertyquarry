...
def test_uri_format_backend_is_active_and_rejects_malformed_uris() -> None:
    ...
    with pytest.raises(ValidationError):
        validator.validate("https://authority.invalid/%ZZ")
    with pytest.raises(ValidationError):
        validator.validate("https://[:::]/v2")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "malformed-percent-endpoint",
        "malformed-ipv6-endpoint",
...
def test_controller_schema_rejects_hostile_config_mutations(mutation: str) -> None:
    ...
    elif mutation == "malformed-percent-endpoint":
        value["authorities"]["request"]["endpoint"] = (  # type: ignore[index]
            "https://authority.invalid/%ZZ"
        )
    elif mutation == "malformed-ipv6-endpoint":
        value["authorities"]["request"]["endpoint"] = (  # type: ignore[index]
            "https://[:::]/v2"
        )
