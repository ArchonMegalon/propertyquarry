...
## Repository release-verifier Python

Repository-side flagship checks run through
`scripts/propertyquarry_release_python.sh`. The launcher rejects
`PYTHON_BIN` and `PYTEST_PYTHON_BIN`, clears Python, pytest, and dynamic-loader
control variables, and accepts only the CPython binary, complete root-owned
standard-library tree, external `sitecustomize` and libpython targets,
dependency lock, and private verifier tree bound by
`config/propertyquarry_release_python_pin.json`. The dependency environment is
built with `--require-hashes`, without user-site packages or mutable bytecode,
by `make bootstrap-propertyquarry-release-python`.

The v2 tree commitment includes the root, empty directories, file type,
permission mode, owner class, regular-file bytes, and an exact allowlist of
symlink targets. Hashing opens regular files with `O_NOFOLLOW`, compares
descriptor and pathname identity before and after reading, and repeats the
tree snapshot. The private environment lives below the owner-only
`.propertyquarry_release_tools/` directory in the canonical
`/docker/property` checkout. Changing an installed file or directory, adding
an unexpected symlink, enabling peer writes, or drifting the interpreter or
requirements lock makes the launcher fail before a release validator starts.

`make all-local`, `make ci-gates`, and the unqualified asset/readiness targets
are explicitly non-authoritative and may use a configurable development
interpreter. The authenticated command
`./scripts/propertyquarry_release_python.sh scripts/propertyquarry_release_make_dispatch.py release-preflight`
instead dispatches the `*-authenticated` targets through the pinned launcher.

This verifier protects repository checks from interpreter substitution and
dependency drift. It does not elevate candidate-side validation into production
authority; the installed controller and external authorities described above
remain the only production mutation boundary. The root-owned libc, dynamic
loader, and transitive shared-library base are trusted host prerequisites, not
part of this repository pin. A process with the same UID can also race
candidate-side files after verification; only the separately installed,
root-owned controller closes that boundary.
