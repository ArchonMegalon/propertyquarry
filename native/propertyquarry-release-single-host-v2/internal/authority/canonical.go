...
func decodedJSONObject(raw []byte, maximum int) (map[string]any, error) {
	if len(raw) == 0 || len(raw) > maximum {
		return nil, fmt.Errorf("json-size-invalid")
	}
	decoder := json.NewDecoder(bytes.NewReader(raw))
...
func writeCanonical(output *bytes.Buffer, value any, depth int) error {
	...
	case string:
		writeCanonicalString(output, item)
	case json.Number:
		text := item.String()
...
// writeCanonicalString matches Python json.dumps(..., ensure_ascii=True,
// separators=(",", ":")), which is the package and helper wire contract.
func writeCanonicalString(output *bytes.Buffer, value string) {
	const hexDigits = "0123456789abcdef"
	writeUnicodeEscape := func(character uint16) {
		output.WriteString(`\u`)
		output.WriteByte(hexDigits[(character>>12)&0xf])
		output.WriteByte(hexDigits[(character>>8)&0xf])
		output.WriteByte(hexDigits[(character>>4)&0xf])
	...
	for _, character := range value {
		...
		default:
			if character < 0x20 || character > 0x7e {
				if character <= 0xffff {
					writeUnicodeEscape(uint16(character))
				} else {
					high, low := utf16.EncodeRune(character)
					writeUnicodeEscape(uint16(high))
					writeUnicodeEscape(uint16(low))
				}
				continue
			}
			output.WriteRune(character)
