...
- [ ] For PropertyQuarry, review the local deploy receipt plus release-bound, sanitized API/database log and monitoring evidence. Only the clean canonical checkout's authenticated local deploy entrypoint has Docker release authority; ad-hoc Compose commands and raw logs are not release evidence.
...
- [ ] Follow `docs/PROPERTYQUARRY_IMAGE_PUBLICATION.md`; build the web and render images from the clean canonical checkout on the local Docker daemon, retain their immutable image IDs in the deployment receipt, and treat those IDs as image evidence only—not proof that deployment is healthy.
...
- [ ] Confirm the local security gate has reviewed pinned `pip-audit`, Syft, and Trivy versions, current pre-provisioned Trivy databases, and both exact images already loaded locally.
- [ ] Require the focused local flagship-security phase to pass before deployment; missing scanners, images, databases, SBOMs, or scanner JSON must fail closed.
- [ ] Require its mode-`0600` receipt to bind the exact runtime SHA, immutable local image IDs, tool/database identities, completion status, and artifact hashes. Missing, stale, or mismatched evidence blocks release even when an individual scanner exited successfully.
...
- [ ] Preserve the complete private local security receipt set and do not replace flagship evidence with an advisory scan.
...
- [ ] Treat the local operator process and Docker daemon as the release-probe secret boundary. Load secrets only from ignored mode-`0600` local inputs or the already governed container environment; never print, persist, interpolate into arguments, or copy them into receipts. Clear shell, loader, and language hooks before privileged gate execution, and require the local host/runtime hardening receipt to bind the exact candidate and images.
...
- [ ] Run the authoritative local Compose deployment only after the terminal Gold command passes. Re-prove the exact candidate at the public runtime before making or publishing a global flagship claim.
