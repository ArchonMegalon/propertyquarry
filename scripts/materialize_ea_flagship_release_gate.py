...
def browser_receipt_pass_blockers(receipt: dict[str, Any], seed: dict[str, Any]) -> list[str]:
    ...
    if type(raw_sources) is not list:
        blockers.append("current gate seed browser evidence sources must be a governed list")
        return blockers
    sources = [entry for entry in raw_sources if type(entry) is dict]
...
def build_receipt(
    ...
    *,
    ...
    blockers: list[str] = []
    if not seed_present or not seed:
        blockers.append(f"flagship gate seed is missing or invalid: {seed_path.as_posix()}")
    if type(seed.get("browser_workflow_proof")) is not dict:
        blockers.append("flagship gate seed browser_workflow_proof is missing or invalid")
    if type(seed.get("release_claim")) is not dict:
        blockers.append("flagship gate seed release_claim is missing or invalid")
    if type(seed.get("version")) is not int:
        blockers.append("flagship gate seed version is missing or invalid")
