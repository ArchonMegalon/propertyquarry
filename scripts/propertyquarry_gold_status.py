...
def _receipt_freshness_status(
    ...
    *,
    ...
    for area, payload in receipts.items():
        ...
        if age_seconds < -DEFAULT_RECEIPT_FUTURE_TOLERANCE_SECONDS:
            rows.append(
                {
                    "area": area,
                    ...
                    "max_future_seconds": (
                        DEFAULT_RECEIPT_FUTURE_TOLERANCE_SECONDS
                    ),
                }
            )
            continue
...
def build_gold_status_receipt(
    *,
    ...
    if max_receipt_age_hours is None:
        normalized_max_receipt_age_hours = None
    else:
        try:
            normalized_max_receipt_age_hours = float(max_receipt_age_hours)
        except (TypeError, ValueError) as exc:
            raise ValueError(
                "max_receipt_age_hours must be a finite number"
            ) from exc
        if not math.isfinite(normalized_max_receipt_age_hours):
            raise ValueError("max_receipt_age_hours must be a finite number")
    effective_max_receipt_age_hours = (
        DEFAULT_FLAGSHIP_MAX_RECEIPT_AGE_HOURS
        if flagship_profile
        and (
            normalized_max_receipt_age_hours is None
            or normalized_max_receipt_age_hours <= 0
        )
        else normalized_max_receipt_age_hours
    )
